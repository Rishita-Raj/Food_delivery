  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-6 space-y-6">
        ...
        <div className="flex gap-4">
            <button onClick={()=>setTab("restaurant")} className={`px-4 py-2 rounded ${tab === "restaurant" ? "bg-red-500 text-white" : "bg-gray-200 text-black"}`}>
                Restaurant
            </button>
            <button onClick={()=>setTab("rider")}
             className={`px-4 py-2 rounded ${tab === "rider" ? "bg-red-500 text-white" : "bg-gray-200 text-black"}`}>
                Riders 
            </button>
        </div>

        {
            tab==='restaurant' && ( <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {restaurant.length === 0 ? (<p>No Pending restaurants</p> 
                  ...
                  restaurant.map((r) => <AdminRestaurantCard key={r._id} restaurant={r} onVerify={fetchData}/>)  
                    )}
            </div>
        )}
