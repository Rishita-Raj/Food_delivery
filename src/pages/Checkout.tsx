...
const Checkout = () => {
  ...
  <div className="mx-auto max-w-4xl px-4 py-6 space-y-6">
    ...
    <div className="rounded-xl bg-white p-4 shadow-sm space-y-3">
      ...
      {
        loadingAddress ? <p className="text-sm text-gray-500">Loading addresses...</p> : addresses.length === 0 ? <p className="text-sm text-gray-500">No address found. Please add one </p> : addresses.map((add)=>(
          <label key={add._id} className={`flex gap-3 rounded-lg border p-3 cursor-pointer transition ${
            selectedAddressId === add._id ? "border-[#e23744] bg-red-50"
            : "hover:bg-gray-50"
          }`}>
    ...
    <div className="rounded-xl bg-white p-4 shadow-sm space-y-4 ">
      ...
      <div className="flex justify-between text-sm">
        ...
        <span>{deliveryFee === 0 ? "Fee" : `₹${deliveryFee}`}</span>
      </div>
      <div className="flex justify-between text-sm">
