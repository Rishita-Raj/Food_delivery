...
const RiderDashboard = () => {
    ...
    navigator.geolocation.getCurrentPosition(async(pos)=>{
        try {
            await axios.patch(`${riderService}/api/rider/toggle`,{
                isAvailible : !profile?.isAvailible,
            ...
            toast.success(profile?.isAvailible ? "You are offline" : "You are online");
            fetchProfile(); 
  ...
  if(!profile) return (  <div className="min-h-screen bg-gray-50 px-4 py-6">
    ...
    <div className="space-y-4">
      <div className="mx-auto max-w-md px-4 py-4">
        ...
        <div className="flex justify-center gap-2"> 
            <span className="px-2 py-1 text-xs rounded-full bg-gray-100 text-green-600">
                {profile.isVerified ? "Verifies" : "Pending"}
            </span>

            <span className="px-2 py-1 text-xs rounded-full bg-gray-100 text-green-600">
                {profile.isAvailible ? "Online" : "Offline"}
            </span>
        </div>
        ...
        {profile.isVerified && !currentOrder && (<button onClick={toggleAvailibility} disabled={toggling} className={`w-full py-2 rounded-lg text-white font-semibold ${toggling?"bg-gray-400" : profile.isAvailible? "bg-gray-600" : "bg-[#e23744]"}`}>
            {
                toggling ? "Updating..." : profile.isAvailible ? "Go Offline" : "Go Online"
            }
            </button>
        )}
        </div>
       ...
       {!audioUnlocked && (<div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-center justify-between">
        ...
        <button onClick={unlockAudio} className="bg-blue-600 hover:bg-blue-400 text-white px-4 py-2 rounded-lg font-medium transition">Enable sound</button>
         </div>
         
         )}


         {profile.isAvailible && incomingOrders.length>0 && (
