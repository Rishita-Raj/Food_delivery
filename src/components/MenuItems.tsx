            ...
            <div className="flex flex-1 flex-col justify-between">
                ...
                <div className="flex items-center justify-between gap-4">
                  <p className="font-medium">₹{item.price}</p>
                  {
                    ...
                    {item.isAvailable ? (<BsEye size={18}/>
        ):(
        <FiEyeOff size={18}/>
        )}
                      </button>
                      ...
                      <button onClick={()=> handleDelete(item._id)}
                       className="rounded-lg p-2 text-red-500 hover:bg-red-50"><BiTrash size={18}/>
                      </button>
                    </div>
                  }
                  {
                    !isSeller && ( <button disabled={!item.isAvailable || isLoading} 
                      onClick={()=> addToCart(item.restaurantId, item._id)} className={`flex items-center justify-center rounded-lg p-2 ${!item.isAvailable || isLoading
